...
- (void) show{
    ...
    //NSDictionary *attributes = @{NSFontAttributeName:font,cgzi:CGSizeMake(280, 60)};
    //CGSize textSize  = [text sizeWithAttributes:attributes];
    
	UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, textSize.width, textSize.height + 20)];
	...
	if (theSettings.gravity == iToastGravityTop) {
		...
		point = CGPointMake(window.frame.size.width/2, 120);
	}else{
	...
	NSTimer *timer1 = [NSTimer 
					   timerWithTimeInterval:((float)theSettings.duration)/500
					   target:self 
